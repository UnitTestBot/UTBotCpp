...
KleeRunner::createKleeParams(const tests::TestMethod &testMethod, const tests::Tests &tests) {
    fs::path kleeOut = Paths::kleeOutDirForEntrypoints(projectContext, projectTmpPath, tests.sourceFilePath);
...
void KleeRunner::processBatchWithoutInteractive(const std::vector<tests::TestMethod> &testMethods,
    ...
    for (const auto &testMethod : testMethods) {
        ...
        auto [argvData, kleeOut] = createKleeParams(testMethod, tests);
